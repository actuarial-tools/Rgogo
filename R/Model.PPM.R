...
setMethod(
   ...
   definition = function(object, var, result = list()) {
      ...
      result1$Res.Gross <- -result1$Pv.Total.Gross
      result1$Res.Rein <- -result1$Pv.Total.Rein
      result1$Res.Net <- max(-result1$Pv.Total.Net, resFloor)
      ...
      if (GetArgValue(object, "ApplyLapseMargin")) {
         ...
         result2$Res.Gross <- -result2$Pv.Total.Gross
         result2$Res.Rein <- -result2$Pv.Total.Rein
         result2$Res.Net <- max(-result2$Pv.Total.Net, resFloor)
         result2$.LapsePfadUsed <- GetLapsePfad(GetArgValue(model.dcf, "LapseAssump"))
         if (result2$Res.Net > result1$Res.Net) {
            result <- result2
         } else {
            result <- result1
         }
...
.SumrzResult.Model.PPM <- function(model, cov, result) {
   ...
   df <- data.frame(
      ...
      CV = curCV,
      CVDfcn = curCV - result$Res.Net,
      LiabDur = GetProjLen(result$Timeline),
      Pv.Prem = ifelse(is.null(result$Pv.Prem), 0, result$Pv.Prem),
      Pv.Prem.Tax = ifelse(is.null(result$Pv.Prem.Tax), 0, result$Pv.Prem.Tax),
      Pv.Comm = ifelse(is.null(result$Pv.Comm), 0, result$Pv.Comm),
      Pv.Comm.Ovrd = ifelse(is.null(result$Pv.Comm.Ovrd), 0, result$Pv.Comm.Ovrd),
      Pv.Ben.Dth = ifelse(is.null(result$Pv.Ben.Dth), 0, result$Pv.Ben.Dth),
      Pv.Ben.Mat = ifelse(is.null(result$Pv.Ben.Mat), 0, result$Pv.Ben.Mat),
      Pv.Ben.Sur = ifelse(is.null(result$Pv.Ben.Sur), 0, result$Pv.Ben.Sur),
      Pv.Ben.Dth.PUA = ifelse(is.null(result$Pv.Ben.Dth.PUA), 0, result$Pv.Ben.Dth.PUA),
      Pv.Ben.Mat.PUA = ifelse(is.null(result$Pv.Ben.Mat.PUA), 0, result$Pv.Ben.Mat.PUA),
      Pv.Ben.Sur.PUA = ifelse(is.null(result$Pv.Ben.Sur.PUA), 0, result$Pv.Ben.Sur.PUA),
      Pv.Ben.Anu = ifelse(is.null(result$Pv.Ben.Anu), 0, result$Pv.Ben.Anu),
      Pv.Expns.Acq = ifelse(is.null(result$Pv.Expns.Acq), 0, result$Pv.Expns.Acq),
      Pv.Expns.Mnt = ifelse(is.null(result$Pv.Expns.Mnt), 0, result$Pv.Expns.Mnt),
      Pv.Rein.Ben = ifelse(is.null(result$Pv.Rein.Ben), 0, result$Pv.Rein.Ben),
      Pv.Rein.Prem = ifelse(is.null(result$Pv.Rein.Prem), 0, result$Pv.Rein.Prem),
      Pv.Rein.Prem.Rfnd = ifelse(is.null(result$Pv.Rein.Prem.Rfnd), 0, result$Pv.Rein.Prem.Rfnd),
      Pv.Rein.Comm = ifelse(is.null(result$Pv.Rein.Comm), 0, result$Pv.Rein.Comm),
      Pv.Rein.Comm.Rfnd = ifelse(is.null(result$Pv.Rein.Comm.Rfnd), 0, result$Pv.Rein.Comm.Rfnd),
      stringsAsFactors = FALSE
   )
   return(df)
}
